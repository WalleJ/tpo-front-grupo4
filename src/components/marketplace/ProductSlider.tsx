...
function getHubLabel(product: Product) {
...
function getAssistantLabel(product: Product) {
  return ASSISTANT_LABELS[product.assistantKey] ?? product.assistantKey;
}

export function ProductSlider({ title, products, onSelect }: Readonly<ProductSliderProps>) {
    ...
    <div className="relative group">
      <div className="flex justify-between items-center mb-md">
        ...
        <div className="flex gap-xs">
          ...
          >
            <span className="material-symbols-outlined text-[20px] text-primary leading-none">chevron_left</span>
          </button>
          <button
          ...
          >
            <span className="material-symbols-outlined text-[20px] text-primary leading-none">chevron_right</span>
          </button>
        </div>
      </div>
      <div className="slider-container flex gap-md overflow-x-auto snap-x snap-mandatory pb-4">
        {products.map((product, i) => (
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-sm border-t border-outline-variant/20 pt-sm mt-sm">
              <div className="space-y-[2px]">
                <p className="text-[10px] uppercase font-bold text-on-surface-variant opacity-60 tracking-wide">ASSISTANT</p>
                <p className="text-body-sm font-medium text-on-surface">{getAssistantLabel(product)}</p>
              </div>
              <div className="space-y-[2px]">
                <p className="text-[10px] uppercase font-bold text-on-surface-variant opacity-60 tracking-wide">HUB</p>
                <p className="text-body-sm font-medium text-on-surface">{getHubLabel(product)}</p>
              </div>
            </div>
          </button>
        ))}
