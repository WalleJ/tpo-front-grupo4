    ...
    <section className="max-w-[1280px] mx-auto px-gutter pb-xl">
      <div className="bg-surface-container-highest rounded-[32px] p-lg md:p-xl flex flex-col md:flex-row items-center gap-lg">
        ...
        <div className="w-full md:w-[440px] z-10">
          <form className="flex flex-col sm:flex-row gap-sm w-full" onSubmit={handleSubmit}>
            <input name="email" className="bg-surface-container-lowest border border-outline-variant/50 px-md py-sm rounded-xl w-full md:w-80" placeholder="email@domain.com" type="email" required />
          ...
          <p className={cn('mt-sm text-body-sm min-h-[22px]', isError ? 'text-error' : 'text-primary')}>{message}</p>
        </div>
      </div>
    </section>
  );
