...
interface ProductMeta {
  ...
  assistant: string;
  ecosystem: string;
  technical: { chip: string; aiEngine: string; memory: string };
  connectivityDetails: { wifi: string; bluetooth: string; thread: string };
  power: { input: string; standby: string; connector: string };
}
...
export function ProductDetail({ product, onAddToCart }: Readonly<ProductDetailProps>) {
  ...
  return (
    <main className="pt-10 pb-32 max-w-[1280px] mx-auto px-gutter">
      <nav className="mb-md flex items-center gap-xs">
        <button
          type="button"
        ...
        >
          ...
          <span className="font-label-caps text-label-caps">BACK TO MARKETPLACE</span>
        </button>
      </nav>
        ...
        <div className="lg:col-span-7 space-y-md">
          <div className="glass-panel rounded-xl overflow-hidden relative aspect-square md:aspect-[4/3] flex items-center justify-center p-lg">
            ...
            <div className="absolute top-md right-md flex flex-col gap-sm">
              <div className="glass-panel px-sm py-xs rounded-full flex items-center gap-xs">
                <span className={`w-2 h-2 rounded-full ${product.inStore ? 'bg-primary-fixed-dim animate-pulse' : 'bg-error'}`} />
                <span className={`font-label-caps text-label-caps ${product.inStore ? 'text-primary' : 'text-error'}`}>{stockLabel}</span>
              </div>
            </div>
          </div>

          <div className="grid grid-cols-4 gap-md">
            <button type="button" className="glass-panel rounded-lg aspect-square p-sm border border-primary cursor-default" aria-label="Selected product image">
              <img className="w-full h-full object-cover rounded" src={product.image} alt={product.title} />
            </button>
            <div className="glass-panel rounded-lg aspect-square p-sm border border-transparent hover:border-outline-variant transition-colors flex items-center justify-center">
              <span className="material-symbols-outlined text-outline">photo</span>
            </div>
            <div className="glass-panel rounded-lg aspect-square p-sm border border-transparent hover:border-outline-variant transition-colors flex items-center justify-center">
              <span className="material-symbols-outlined text-outline">photo</span>
            </div>
        ...
        <div className="lg:col-span-5 space-y-lg">
          ...
          <div className="grid grid-cols-2 gap-md">
            {compactSpecs.map((spec) => (
              <div key={`${product.id}-${spec.label}`} className="glass-panel p-md rounded-xl space-y-xs">
                <span className="material-symbols-outlined text-primary">{spec.icon}</span>
                <div className="font-label-caps text-label-caps text-outline">{spec.label}</div>
                <div className="font-headline-sm text-headline-sm">{spec.value}</div>
              </div>
            ))}
          </div>

          <section className="space-y-md">
            <div className="font-label-caps text-label-caps text-outline-variant">SYSTEM COMPATIBILITY</div>
            <div className="flex flex-wrap gap-md items-center opacity-60">
              <div className="flex items-center gap-xs"><span className="material-symbols-outlined text-[24px]">settings_voice</span><span className="font-label-caps text-[10px]">VOICE AI</span></div>
              <div className="flex items-center gap-xs"><span className="material-symbols-outlined text-[24px]">record_voice_over</span><span className="font-label-caps text-[10px]">MULTI-ASSISTANT</span></div>
              <div className="flex items-center gap-xs"><span className="material-symbols-outlined text-[24px]">smart_toy</span><span className="font-label-caps text-[10px]">VOICE CONTROL</span></div>
            </div>
          </section>
          ...
          <div className="space-y-md pt-md">
            ...
            >
              <span>{product.inStore ? 'Add to Cart' : 'Out of stock'}</span>
      ...
      <div className="mt-xl glass-panel rounded-xl overflow-hidden">
        <div className="border-b border-outline-variant/20 p-md flex items-center justify-between">
          <h3 className="font-headline-sm text-headline-sm">Detailed Technical Specification</h3>
          <button
            type="button"
          ...
          >
            <span className="material-symbols-outlined">{isTechSpecsOpen ? 'expand_less' : 'expand_more'}</span>
          </button>
        </div>
        ...
        {isTechSpecsOpen ? (
          <div className="p-lg grid grid-cols-1 md:grid-cols-3 gap-lg">
            <div className="space-y-sm">
              <h4 className="font-label-caps text-label-caps text-primary">TECHNICAL SPECS</h4>
              <ul className="space-y-xs text-body-sm text-on-surface-variant">
                <li className="flex justify-between border-b border-outline-variant/10 pb-xs"><span>Chip</span><span>{meta.technical.chip}</span></li>
                <li className="flex justify-between border-b border-outline-variant/10 pb-xs"><span>AI Engine</span><span>{meta.technical.aiEngine}</span></li>
                <li className="flex justify-between border-b border-outline-variant/10 pb-xs"><span>Memory</span><span>{meta.technical.memory}</span></li>
              </ul>
            </div>

            <div className="space-y-sm">
              <h4 className="font-label-caps text-label-caps text-primary">CONNECTIVITY</h4>
              <ul className="space-y-xs text-body-sm text-on-surface-variant">
                <li className="flex justify-between border-b border-outline-variant/10 pb-xs"><span>Wi-Fi</span><span>{meta.connectivityDetails.wifi}</span></li>
                <li className="flex justify-between border-b border-outline-variant/10 pb-xs"><span>Bluetooth</span><span>{meta.connectivityDetails.bluetooth}</span></li>
                <li className="flex justify-between border-b border-outline-variant/10 pb-xs"><span>Thread</span><span>{meta.connectivityDetails.thread}</span></li>
              </ul>
            </div>

            <div className="space-y-sm">
              <h4 className="font-label-caps text-label-caps text-primary">POWER</h4>
              <ul className="space-y-xs text-body-sm text-on-surface-variant">
                <li className="flex justify-between border-b border-outline-variant/10 pb-xs"><span>Power Input</span><span>{meta.power.input}</span></li>
                <li className="flex justify-between border-b border-outline-variant/10 pb-xs"><span>Standby Power</span><span>{meta.power.standby}</span></li>
                <li className="flex justify-between border-b border-outline-variant/10 pb-xs"><span>Connector</span><span>{meta.power.connector}</span></li>
              </ul>
            </div>
          </div>
        ) : null}
